...
void runTaskFirstTime(int i){
	...
	pxCurrentTCB = (long)(taskTable[currentTask].stack + TASK_STACK_SIZE - 1);
...
int findNextTask()
{
	...
	// mark the current task as being runned
	if(currentTask >= 0)	taskTable[currentTask].runflag = 1;
	return ((currentTask+1)%task_counter);	// round robin
}
void OSSwapTask()
 {
	if(!calledByOS)return;	// disable this if it's not called by OS
	else	calledByOS = 0;
	sei();
	...
	if(taskNum == -1)	return;	// no task to swap
	...
	TTaskBlock *t = &taskTable[taskNum];
