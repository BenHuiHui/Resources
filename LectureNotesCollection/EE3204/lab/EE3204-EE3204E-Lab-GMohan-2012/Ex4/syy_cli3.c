...
int main(int argc, char **argv){
  ...
  int i,j;
  int data_len_base = 100;
  for(i = 1; i <= 10; i++){
    double trans_time_sum;
    data_len = i * data_len_base;
    ...
    for(j = 0; j < 10; j++){
      ...
      if(j == 9){ // save and write
        ...
        fprintf(fdata,"%ld\t%.3f\t%f\n", data_len,trans_time,through_put);
      }
...
double send_file(FILE *fp, int sockfd, long *len){
  ...
  if( recv_ret == -1){
    fprintf(stderr,"Didn't got response, timeout. Exiting.\n");
    exit(1);
  } else if(ack.num != 1 || ack.len != 0){
    fprintf(stderr,"Error when receiving ack. Exiting.\n");
    exit(1);
  } 
