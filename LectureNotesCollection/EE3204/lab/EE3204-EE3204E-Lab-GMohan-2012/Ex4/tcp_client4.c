...
int main(int argc, char **argv)
{
	...
	bzero(&(ser_addr.sin_zero), 8); // Er.. note that we actually don't need the & here
  ...
  recordFile = fopen("data.txt","w");
  fprintf(recordFile,"Time(ms)\tData Sent(Byte)\tData Rate (Kbytes/s)\n");
...
// allocate memory to contain the whole file.
  ...
  /*** the whole file is loaded in the buffer. ***/
    ...
    if (ack.num != 1|| ack.len != 0) printf("error in transmission\n");
    else printf("Confirm Received.\n");
		ci += slen;
	}
