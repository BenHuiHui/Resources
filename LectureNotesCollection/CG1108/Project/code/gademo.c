...
void set_Left_Forward(char phase){
  PWM1_ChangeDutyCycle(phase,0x3F);
  RD0 = 0;
}
...
void set_Right_Stop(){
  PWM2_ChangeDutyCycle(0,0x3F);
  RD1 = 0;
}
void set_Right_Forward(char phase){
  PWM2_ChangeDutyCycle(phase,0x3F);
  RD1 = 0;
}
...
while(1)
{
	...
	if(!RB7 & !RB6 & !RB5){			// 000 (just right, go forward)
	  set_Left_Forward(base);
	  set_Right_Forward(base);
	} else if(!RB7 & !RB6 & RB5){	// 001 (right encounter white, turn left)
	  set_Left_Stop();
	  set_Right_Forward(base);
	} else if(RB7 & !RB6 & !RB5){	// 100 (left encounter white, turn right)
	  set_Left_Forward(base);
	  set_Right_Stop();
	} else if(RB7 & !RB6 & RB5){	// 101 (unwanted case, stop)
	  set_Left_Stop();
	  set_Right_Stop();
	} else if(!RB7 & RB6 & !RB5){	// 010 (narrow way, forward)
	  set_Left_Forward(base);
	  set_Right_Forward(base);
	} else if(RB7 & RB6 & RB5){		// 111 (totally out, stop)
	  set_Left_Stop();
	  set_Right_Stop();
	} else if(!RB7 & RB6 & RB5){	// 011 (small turn to left)
	  set_Left_Back(base);
	  set_Right_Forward(base);
	} else{							// 110 (small turn to right)
	  set_Left_Forward(base);
	  set_Right_Back(base);
	}
}
