% Changeds Made:
%  1. Added an operator for "for" statement (line 9)
%  2. Added predicate to handle the for..do.. statement (line 115~134)
:- op(1099,yf,;).
:- op(960,fx,if).
:- op(959,xfx,then).
:- op(958,xfx,else).
:- op(960,fx,while).
:- op(960,fx,for). % define operator for "for"
:- op(959,xfx,do).
...
compile(for(A;B;C) do S,Ein,Eout,Tin,Tout,Lin,Lout) :- !,
	compile(A,Ein,Ein_1,Tin,Tin_1,Lin,Lin), % the initialization
	% the condition
	B =.. [O,X,Y], Lin_1 is Lin+1,
	(   O == (\=) -> Otrans = '!='; Otrans = O),
	write('Lfor'),write(Lin),writeln(':'),
	compileExpr(X,Ein_1,Ein_2,Tin_1,Tin_2),
	compileExpr(Y,Ein_2,Ein_3,Tin_2,Tin_3),
	writeln('    ecx = *(int*)&M[esp]; esp += 4;'),
	writeln('    eax = *(int*)&M[esp]; esp += 4;'),
	write('    if (eax '), write(Otrans),
	write(' ecx ) goto Lforbody'),write(Lin),writeln(';'),
	write('    goto Lendfor'),write(Lin), writeln(';'),
	% the loop
	write('Lforbody'),write(Lin),writeln(':'),
	compile(S,Ein_3,Ein_4,Tin_3,Tin_4,Lin_1,Lout),
	% put in the increment of the for loop here
	compile(C,Ein_4,Eout,Tin_4,Tout,Lout,Lout),
	write('    goto Lfor'),write(Lin),writeln(';'),
	write('Lendfor'),write(Lin),writeln(:).
