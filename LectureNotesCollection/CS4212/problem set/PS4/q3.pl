...
% 4th arg: array of pointers to each of the strings of variable name
% 5th arg: allocation of space for each array
% 6th arg: array of strings containing names of array
% 7th arg: array of pointers of each of the strings of array name
% 8th arg: array of variables containing the size of each array
allocvars([],[],[],[],[],[],[],[]).
allocvars([V@Size|VT],VCL,VNL,VPL,[AC|ACT],[AN|ANT],[AP|APT],[AS|AST]) :-!,
    ByteSize #= Size * 4,
	atomic_list_concat(['\n',V,':\t\t .space ',ByteSize],AC),
	atomic_list_concat(['\n',V,'_aname:\t .asciz "',V,'"'],AN),
	atomic_list_concat(['\n',V,'_aptr:\t .long ',V,'_aname'],AP),
	atomic_list_concat(['\n',V,'_asize:\t\t .long ',Size],AS),
	allocvars(VT,VCL,VNL,VPL,ACT,ANT,APT,AST).


allocvars([V|VT],[VC|VCT],[VN|VNT],[VP|VPT],ACL,ANL,APL,ASL) :-
	atomic_list_concat(['\n',V,':\t\t .long 0'],VC),
	atomic_list_concat(['\n',V,'_name:\t .asciz "',V,'"'],VN),
	atomic_list_concat(['\n',V,'_ptr:\t .long ',V,'_name'],VP),
	allocvars(VT,VCT,VNT,VPT,ACL,ANL,APL,ASL).
...
ce(X@I,Code,Ain,Aout) :- % generate code for array access
    ...
    % EDIT - start
	(	member((X@_,Ref),Locals)
     ;  member((X,Ref),Locals)
	 ;	member(X@_,Globals), Ref = X
...
move_result_to_var(Attr,InVar,Code) :-
    ...
    (   InVar = Res@L
    ->  comp_expr(L,CE,Attr,AttrO),
        move_result_to_reg(AttrO,RReg,MRegC),
        (   RReg = '%ebx'  -> AddrReg = '%eax' ; AddrReg = '%ebx'),PopToR = RReg,
        append([CE,MRegC,
            ['\n\t\t leal ',Res,',',AddrReg,
             '\n\t\t imull $4,',RReg,
             '\n\t\t addl ',RReg,',',AddrReg]],Mid),
        atomic_list_concat(['(',AddrReg,')'],Var)
    ;   Var = InVar, Mid = [],PopToR = '%eax'),
    append([Pre,Mid,Post],Code).
...
local_vars_helper(V,Ain,Aout) :-
    ...
    ( V =.. [@,_,Size] -> Shift #= 4 * Size ; Shift = 4),
	TopOut #= TopIn + Shift, atomic_list_concat([-,TopOut,'(%ebp)'],Ref),
...
cs_arr_asign_unroll( (H,T), V, Code, A, Cnt):-
    put_assoc(context,A,expr,A1),    % request expression context
    comp_expr(H,CE,A1,A2),             % compile right hand side

    % Check where the result of rhs is stored, and transfer it into
    % storage of variable X
    move_result_to_var(A2,V@Cnt,CMOV),
    append(CE,CMOV,CodeH),
    NewCnt is Cnt + 1,
    ( T = (_,_) -> cs_arr_asign_unroll( T, V, CodeT, A2, NewCnt)
    ;
    put_assoc(context,A2,expr,A3),    % request expression context
    comp_expr(T,CTE,A3,A4),             % compile right hand side
    move_result_to_var(A4,V@NewCnt,CTMOV), append(CTE,CTMOV,CodeT)
    ),
...
% NOTE: started -- eidt as part of solution
cs((A@(E1:E2) = B@(E3:_)),Code,Ai,Ao) :-!,
    get_assoc(local_vars,Ai,Locals),
    get_assoc(global_vars,Ai,Globals),

    (   member((A@_,AV),Locals);
        member((A,AV),Locals);
        member(A@_,Globals),AV = A;
        member(A,Globals),AV = A
    ),!,
    (   member((B@_,BV),Locals);
        member((B,BV),Locals);
        member(B@_,Globals),BV = B;
        member(B,Globals),BV = B
    ),!,

    put_assoc(context,Ai,expr,As),    % request expression context

    comp_expr(E1,CE1,As,A1),
    move_result_to_reg(A1,Reg1,ME1),
    append([CE1,ME1, ['\n\t\t pushl ',Reg1]],Code1),

    comp_expr(E2,CE2,As,A2),
    move_result_to_reg(A2,Reg2,ME2),
    append([CE2,ME2,['\n\t\tpushl ',Reg2]],Code2),
    
    comp_expr(E3,CE3,As,A3),
    move_result_to_reg(A3,Reg3,ME3),
    append([CE3,ME3,['\n\t\tpushl ',Reg3]],Code3),
    % don't care about the last range.
    get_assoc(labelsuffix,Ai,LNin,Ao,LNo),
    generateLabels([LS,LE],LNin,LNo),

    % copy out and perform the memcpy
    append([Code1,Code2,Code3,
        ['\n\t\t leal ',BV,', %esi', % here we make the source
         '\n\t\t popl  %eax',
         '\n\t\t imull $4,%eax',
         '\n\t\t add   %eax,%esi',

         '\n\t\t leal ',AV,', %ecx', % set terminator -- to be ecx
         '\n\t\t popl %eax',
         '\n\t\t imull $4,%eax',
         '\n\t\t add   %eax,%ecx',
         ...
         '\n\t\t add   %eax,%edi',
                            ...
                            % then we perform the memcpy
         '\n',LS,':',
         '\n\t\t cmp %ecx,%edi',
         '\n\t\t jg ',LE,    % when fulfill -- no need to cpy
         '\n\t\t movl (%esi),%eax',
         '\n\t\t movl %eax,(%edi)',
         '\n\t\t addl $4,%esi',
         '\n\t\t addl $4,%edi',
...
% NOTE: ended -- eidt as part of solution
...
cs((X=(H,T)),Code,A,A):- atom(X),!,
    get_assoc(local_vars,A,Locals),
    get_assoc(global_vars,A,Globals),

    (   member((X@_,V),Locals);
        member((X,V),Locals);
        member(X@_,Globals),V = X;
        member(X,Globals),V = X
    ),!,
    cs_arr_asign_unroll( (H,T), V, Code, A, 0).
    

cs((X=E),Code,Ain,Aout) :- 
    ...
    ; X = V@L,
      (	  member((V@_,R),Locals),Ref = R@L,!;
	  member((V,Ref),Locals),!;
          member(V@_,Globals),Ref = V@L,!;
	  member(V,Globals),Ref = V),!
    ),
...
compile(P,File) :-
	...
	allocvars(VarList,VarCode,VarNames,VarPtrs,ArrCode,ArrNames,ArrPtr,ArrSiz),
	...
	atomic_list_concat(ArrCode,WritableArrs),
	write('\n\t\t .globl __arr_area\n__arr_area:\n'),
                                             % Write declarations to output file
	write(WritableArrs),
	                                         % Create array of strings representing
                                             % global variable names, so that vars can
                                             % be printed nicely from the runtime
	atomic_list_concat(ArrNames,WritableArrList),
	write('\n\n\t\t .globl __arr_name_area\n__arr_name_area:\n'),
	write(WritableArrList),
	                                         % Create array of pointers to strings
                                             % so that runtime code doesn't need
                                             % to be changed every time we compile
	atomic_list_concat(ArrPtr,WritableArrPtrs),
	write('\n\n\t\t .globl __arr_ptr_area\n__arr_ptr_area:\n'),
	write(WritableArrPtrs),
	write('\n\n__end_arr_ptr_area:\t .long 0\n'),

	atomic_list_concat(ArrSiz,WritableArrSiz),
	write('\n\n\t\t .globl __arr_size_area\n__arr_size_area:\n'),
	write(WritableArrSiz),
...
:- Program =
(	global i,a@6,min1,min2 ;
    fmin#(void) ::
    { local i,x@6 ;
      i = 4;
      x = ( i + 1 , i + 2, i + 3, i + 4, i + 45, i + 6);
      a@(2:4) = x@(3:5);
	  i = 0 ; min1 = 100 ;
	  while i < 6 do
