/*****************************************************************
  ...
  Note that to avoid the change of precedrence of the built-in operator (:,=), I defined (?,:) pair to have lower priority.
  Therefore, if one want to do things like x = x > 3 ? (x < 1) : (x + 1), the "x < 1" part must be put in bracket.
