...
cs(({S1} '||' {S2}), Code,Ain, Aout):-!,
	...
	cs(S1, C3, Ain, A1),
	randomInsert(C3, Code3, 1),
	C4 = ['\n\t\t jmp join','\n',L,':'],
	cs(S2, C5, A1, A3),
	get_assoc(labelsuffix,A3,LabelSuffixIn,Aout,LabelSuffixOut),
	generateLabels([L],LabelSuffixIn,LabelSuffixOut),
	randomInsert(C5, Code5, 1),
	C6 = ['\n join:',
	      '\n\t\t call single_thread'],
	append([C1,C2, Code3, C4, Code5, C6], Code).
% NOTE: edit ends
...
Generates the following test1.s
