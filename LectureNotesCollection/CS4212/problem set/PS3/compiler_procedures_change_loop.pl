  ...
  Source language features:
	 ...
	 * procedures
	   - definitions of the form
	              procname#(arg_list) :: { body }
		   -- procname is an atom denoting the procedure name
		   -- '#' is the connective between name and argument list, due to
			  Prolog operator constraints
		   -- (arg_list) is a comma separated list of formal arguments;
		      it takes the form (void) if no arguments are to be passed
			  to the procedure
		   -- '::' is the connective joining the procedure header with the body,
		      due to Prolog operator constraints
		   -- { body } is the body of the procedure, representing normal statements
			  with possible local declarations at the top. The formal arguments, as
			  well as any global variable declarations, may be accessed as expected
			  inside the body.
           -- procedure definitions must appear at the top of the
		      program (Pascal style), between 'global' declarations and
			  the main code of the program.
           -- any procedure can call any other procedure, irrespective
		      of the order of the procedure definitions. Recursion and mutual
...
get_ele_from_list([H|[]],_,H):-!.
get_ele_from_list([H|_],1,H):-!.
get_ele_from_list([_|T],N,E):-
    Np is N - 1,
    get_ele_from_list(T,Np,E).
...
cs(break N, Code,A,A):- !,
    ...
    get_assoc(loop_end_lbl,A,List),
    get_ele_from_list(List,N,Loop_end_lbl),
	Code = [ '\n\t\t jmp ',Loop_end_lbl ].
cs(break, Code,A,A):- !,
	        % Generate a label name, and generate a jump to that label
    get_assoc(loop_end_lbl,A,[Loop_end_lbl|_]),
	Code = [ '\n\t\t jmp ',Loop_end_lbl ].

cs(continue N, Code,A,A) :- !,
    get_assoc(loop_start_lbl,A,List),
    get_ele_from_list(List,N,Loop_start_lbl),
	Code = [ '\n\t\t jmp ',Loop_start_lbl ].
cs(continue, Code,A,A) :- !,
	get_assoc(loop_start_lbl,A,[Loop_start_lbl|_]),
	Code = [ '\n\t\t jmp ',Loop_start_lbl ].
...
cs((while B do { S }), Code,Ain,Aout) :- !,
    get_assoc(loop_start_lbl,Ain,OldLoopStart,A0,[NewLoopStart|OldLoopStart]),
    get_assoc(loop_end_lbl,A0,OldLoopEnd,A1,[NewLoopEnd|OldLoopEnd]),
    CLoopStart = [ '\n',NewLoopStart,':' ],
    cs({S},CS,A1,A2),        % Then generate code for body
...
cs(switch E of { CaseList },Code,Ain,Aout) :-
    ...
    get_assoc(label_case_end,Ain,OldCaseLabel,A_1,Lcaseend),
    ...
    get_assoc(loop_end_lbl,A_1,OldLoopEnd,A0,[Lcaseend|OldLoopEnd]),
...
compile(P,File) :-
    ...
    AstartIn = Empty,
    put_assoc(loop_start_lbl,AstartIn,[],AstartOut),
	AendIn = AstartOut,                     % initial 'break' label is none
    put_assoc(loop_end_lbl,AendIn,[],AendOut),
...
:- Prog = (
	 ...
	 d = fibtc#(facttc#(2,1)*facttc#(2,1)+b,facttc#(10,2)/facttc#(10,1),0) ;
	 e = fibtc#(6,1,0) ;
	 ff = f#(10,20,30)*f#(1,1,1) ;
	 gg = g#(void)
		  ), compile(Prog,'proc3.s').

/* Regression tests... check if old tests from previous versions still work */
