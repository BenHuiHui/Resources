...
cs_gen_while_recur((B,S),Code,Ain,Aout):-!,
    get_assoc(unroll_cnt,Ain,CurrentUnrollCnt,Ax,NewUnrollCnt),
    get_assoc(unroll_num,Ax,TotalUnrollCnt),

    get_assoc(break,Ax,OrigBreak,Ax,none),
    ...
    get_assoc(continue,Ax,OrigContinue,Ax,none),
    Cw1 = [ '\n\t\t jmp ',LCond ,
            '\n',LTop,':' ],
                            % Jump to while condition; generate label for looping
    cs({S},C,A1,A2),        % Then generate code for body

    ( ( CurrentUnrollCnt = 1,!,  % First Level
        % Deal with local variables
        cs_filter_local_var(S,Sp,Ax,Ax), % -- Sp is Code without Local Variables

      )
    ; ( CurrentUnrollCnt = TotalUnrollCnt,!, % Last Level

      )
    ; ( CurrentUnrollCnt < TotalUnrollCnt,!, % Middle Level
      )
    ),

    Cw2 = [ '\n',LCond,':' ],
                            % Generate label 'LCond:' for bool condition
    % used to have a B here.
...
cs((while B do { S }), Code,Ain,Aout) :- !,
    ...
    put_assoc(unroll_cnt,Ax,0,Ax),

    % compile these first so that can be used directly by the 'if' generated later
    ...
    (   member((Op,I),[(<,jl),(=<,jle),(==,je),(\=,jne),(>,jg),(>=,jge)])
     ...
     ;  I = jne ),
         % NOTE: edit start
    cs_gen_while_recur( (CB,S), CodeRecur, Ax, Ax),
         % NOTE: edit end
...
compile(P,File,UnrollCnt) :-
    ...
    AbreakLabelIn = AmaxlocalOut,              % current allocation size is 0
	put_assoc(break_target,AbreakLabelIn,none,AbreakLabelOut),
											 % max allocation size is 0
    AunrollCntIn = AmaxlocalOut,
	put_assoc(unroll_num,AunrollCntIn,UnrollCnt,AunrollCntOut),
    Ainit = AunrollCntOut,
