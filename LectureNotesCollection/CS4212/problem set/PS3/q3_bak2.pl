...
cs_filter_out_local_variable(S,SNoLocal,Ain,Aout):-
    (   S = (local VL; Sp)
    ->  (   get_assoc(local_vars,Ain,OriginalLocalVars),    % preserve the old variable
            get_assoc(top_local_vars,Ain,OriginalTopLocalVars),
            local_vars(VL,Ain,A0),
            cs_filter_out_local_variable(Sp,SNoLocal,A0,A1):-
            put_assoc(local_vars,A1,OriginalLocalVars,A2),
            put_assoc(top_local_vars,A2,OriginalTopLocalVars,Aout).
        )
    ;   SNoLocal = S,Ain = Aout)
...
cs((while B do { S }), Code,Ain,Aout) :- 
    ...
    get_assoc(loop_start_lbl,Ain,OldLoopStart,A0,NewLoopStart), % the LoopStart for continue to jump to
    get_assoc(loop_end_lbl,A0,OldLoopEnd,A1,NewLoopEnd),        % this part will always be the same for unwraping.. put it here for consistency

    (   get_assoc(unroll_cnt,Ax,UnrollCnt) -> true ; UnrollCnt = 0), % load unroll count
    put_assoc(unroll_cnt,Ax,NewUnrollCnt),NewUnrollCnt is UnrollCnt + 1,

    (   (   UnrollCnt = 0,!,    % just do initialization

            % compile B code, and save it to A
            put_assoc(context,Ax,stmt,Ax),
                                    % Generate code for boolean condition, in
                                    % 'stmt' context so that residual code is not used.
            comp_expr(B,CB,Ax,Ax),  % Results will be in the flags
            B =.. [Op|_],           % Appropriate conditional jump must be selected
            (   member((Op,I),[(<,jge),(=<,jg),(==,jne),(\=,je),(>,jle),(>=,jl)])
             -> true
             ;  I = je ),
            COpB = ['\n\t\t ',I,' ',NewLoopEnd],    % the NewLoopEnd isn't bound yet so far.
            append(B,COpB,BCode),   % the B code with 1 unbounded variable: NewLoopEnd

            cs({S},SCode,Ax,Ax),     % Then generate code for body. note that when encountered, 
                                     % the NewLoopStartCont and NewLoopEnd are unbounded.
                                     % compiling S once only ensures that the local variables are generated once only

            % the end label will always be the same, therefore generate it first.
            get_assoc(labelsuffix,Ax,L1In,Ax,L1Out),
            generateLabels([NewLoopEnd],L1In,L1Out),
            put_assoc(unroll_compiled_code,Ax,(NewLoopStart,NewLoopEnd,BCode,SCode),Ax), % save the compiled template
            (NewLoopStart,NewLoopEnd,BCode,SCode) = (CLoopStart,CLoopEnd,CBCode,CSCode), % just in case used later  %TODO

            cs((while B do { S }), Code,Ax,Ax)  % after initialization, go to later phase and unroll the code for while loop.
        );
        (   UnrollCnt < UnrollNum,!,
            get_assoc(unroll_compiled_code,Ax,OldCodeTerms), % get the compiled code, and copy over the unbounded start/end tag variables
            term_copy(OldCodeTerms,NCodeTerms),
            OldCodeTerms = (OldContTo,_,OldBCode,OldCode),
            CurrentCodeTerms = (NewLoopStart,NewLoopEnd,CBCode,CSCode),   % Also bound NewLoopEnd flag here.
            
            term_copy((CLoopStart,CSCode),(NLoopStart,NSCode)),
            NextCodeTerms = (NLoopStart,CLoopEnd,CBCode,NSCode),

            get_assoc(labelsuffix,Ax,L2In,Ax,L2Out),
            generateLabels([CLoopStart],L2In,L2Out),
            
            OldContTo = CLoopStart,
            OldBreakTo = CLoopEnd,

            put_assoc(unroll_compiled_code,Ax,CurrentCodeTerms,Ax),

            (   UnrollCnt = 1 % if 1 then no previous need to jump to here, just started.
            ->  CurrentCode = [OldBCode,OldSCode],
            ;   CurrentCode = 
                [   '\n',OldLoopStart,':', OldBCode, OldSCode],
            )
            cs({S},CodeRest,Ax,Ax),
    ...
    ->  append([
        ...
        get_assoc(labelsuffix,Ax,Kin,Ax,Kout),
        generateLabels([CLoopStart,CLoopEnd],Kin,Kout),

        put_assoc(unroll_cnt,Ax,0,Ax)  % in the end, reset the unroll_cnt value
                    % restore the old label, marks the end.
    ;  true
    )

    put_assoc(loop_start_lbl,A5,OldLoopStart,A6),
    put_assoc(loop_end_lbl,A6,OldLoopEnd,Aout).
...
compile(P,File,UnrollNum) :-    % add in variable UnrollNum
	...
	AUnrollNumIn = Empty,
    put_assoc(unroll_num,AUnrollNumIn,none,AUnrollNumOut),
