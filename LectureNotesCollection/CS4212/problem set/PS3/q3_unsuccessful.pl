...
cs(break, Code,A,A):- !,
    ...
    get_assoc(break_to_lbl,A,Loop_end_lbl),
	Code = [ '\n\t\t jmp ',Loop_end_lbl ].

cs(continue, Code,A,A) :- !,
	get_assoc(cont_to_lbl,A,Loop_start_lbl),
	Code = [ '\n\t\t jmp ',Loop_start_lbl ].
...
cs((while B do { S }), Code,Ain,Aout) :-
    ...
    get_assoc(cont_to_lbl,Ain,OldContTarget,A0,NewContTarget), % the LoopStart for continue to jump to
    get_assoc(break_to_lbl,A0,OldBreakTarget,A1,NewBreakTarget),        % this part will always be the same for unwraping.. put it here for consistency

    (   get_assoc(unroll_cnt,A1,UnrollCnt) -> true ; UnrollCnt = 0), % load unroll count
    put_assoc(unroll_cnt,A1,NewUnrollCnt),NewUnrollCnt is UnrollCnt + 1,    % may be overwrite later
    ...
    (   (   UnrollCnt = 0,!,    % just do initialization
            ...
            comp_expr(B,BCode,A3,A4),  % Results will be in the flags
            B =.. [Op|_],           % Appropriate conditional jump must be selected
            (   member((Op,I),[(<,jge),(=<,jg),(==,jne),(\=,je),(>,jle),(>=,jl)])
             -> true
             ;  I = je ),
            COpB = ['\n\t\t ',I,' ',NewBreakTarget],    % The breakTarget is also the end of the loop
            get_assoc(labelsuffix,A4,L1In,A5,L1Out),    % can just generate first directly
            generateLabels([NewBreakTarget],L1In,L1Out),

            append(B,COpB,BCode),   % the B code with 1 unbounded variable: NewLoopEnd

            cs({S},SCode,A5,A6),     % Then generate code for body. note that when encountered, the NewContTarget is
                                     % unbounded. Also note that S is only compile here, compiling S once only ensures
                                     % that the local variables are generated once only

            put_assoc(unroll_compiled_code,A6,(NewContTarget,BCode,SCode),A7), % save the compiled template. The NewContTarget
                                                                               % is the only unbound variable.

            cs((while B do { S }), Code,A7,A8),  % after initialization, go to later phase and unroll the code for while loop.
	    (CurrentContTo,BCode,SCode) = (NewContTarget,CBCode,CSCode)
        );
        (   UnrollCnt < UnrollNum,!,
            get_assoc(unroll_compiled_code,A2,OldCompiledTerms), % get the compiled code, and copy over the unbounded start/end tag variables
            term_copy(OldCompiledTerms,CurrentCompiledTerms),

            OldCompiledTerms = (OldContTarget,OldBCode,OldSCode),   % Also bound NewLoopEnd flag here; NewLoopStart is where the continue would be pointing to
            CurrentCompiledTerms = (CurrentContTo,_,_),

            OldBreakTarget = NewBreakTarget, CurrentContTo = NewContTarget,

            get_assoc(labelsuffix,A2,L2In,A3,L2Out),    %
            generateLabels([OldContTarget],L2In,L2Out),

            (   UnrollCnt = 1 % if 1 then no previous need to jump to here, just started.
            ->  CurrentCode = [OldBCode,OldSCode]    % in that case oldTarget got no use.
            ;   CurrentCode = [   '\n',OldContTarget,':', OldBCode, OldSCode]
            ),
            put_assoc(unroll_compiled_code,A3,CurrentCompiledTerms,A4), % update to make sure that the ContTarget is unbounded.
            cs({S},CodeRest,A4,A5),
            Code = [CurrentCode | CodeRest],
            A8 = A5    % sync the number
        )
    ),
    ...
    ->  append([
        ...
        get_assoc(labelsuffix,A8,Kin,A9,Kout),
        generateLabels([CurrentContTo],Kin,Kout),

        put_assoc(unroll_cnt,A9,0,A10)  % in the end, reset the unroll_cnt value
                    % restore the old label, marks the end.
    ;  true
    ),

    put_assoc(cont_to_lbl,A10,OldContTarget,A11),
    put_assoc(break_to_lbl,A11,OldBreakTarget,Aout).

cs((for (X1=E1;B;X2=E2) do { S }), Code,Ain,Aout) :- !,
    get_assoc(cont_to_lbl,Ain,OldLoopStart,A0,NewLoopStart),
    get_assoc(break_to_lbl,A0,OldLoopEnd,A1,NewLoopEnd),
	                                 % Handle 'break' and 'continue' attrs similar to 'while' rule
	cs(X1=E1,CInit,A1,A2),              % compile first assignment
    CLoopNeck = [ '\n\t\tjmp ',NewLoopMain,
                  '\n',NewLoopStart,':' ],         % Generate 'top-of-loop' label
	...
	CLoopFoot = [ '\n\t\t jmp ',NewLoopStart,      % Generate jump to top of loop, to repeat the loop
                  '\n',NewLoopEnd,':'],       % Generate 'out-of-loop' label placeholder
    ...
    put_assoc(cont_to_lbl,A7,OldLoopStart,A8),
    put_assoc(break_to_lbl,A8,OldLoopEnd,Aout),

    append([CInit,CLoopNeck,CUpdate,CLblMain,CB,COpB,CS,CLoopFoot],Code).
                                     % Lay out the code.
...
% NOTE: edit start
compile(P,File,UnrollNum) :-    % add in variable UnrollNum
    ...
    put_assoc(cont_to_lbl,AstartIn,none,AstartOut),
	AendIn = AstartOut,                     % initial 'break' label is none
    put_assoc(break_to_lbl,AendIn,none,AendOut),
...
:- Program = (
    ...
    while ( i < 100000000) do{
        local s; s = 1; i = i + 1;
        while ( j < 100000000) do{
            s = s + 1; j = j + 1;
        }
    }
), compile(Program,'t2.s',0).
