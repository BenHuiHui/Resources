...
* Changes are made based on "compiler_procedures.pl", which are:
*   1. Change the implemention of if, while, switch, so that for the break/continue,
*      it would use the label passed in directly instead of generating new labels
*   2. For the while/for/switch, instead of replacing the labels for break/continue,
*      make it as a stack instead -- implement it as a list, each time entering a
*      new loop would push label into the list; each time leaving a loop would 
*      restore the old one.
*   3. In the continue N / break N, find the corresponding element in the list,
