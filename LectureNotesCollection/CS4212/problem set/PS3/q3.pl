/********************************************************************************************
  ...
  I've changed lots of parts of the code for this problem so I didn't put any comment beside 
  the changes. Here is a brief description:
  1) As posted in the form, I changed the while loop's compilation to be test condition first,
     and jump on negative (the same for the for loop). Because of this, for the |break| and 
     |continue|, I changed to be use the generated label directly instead of generating labels
     on their own. By doing so we saved some labels in the case when there're lots of break in
     one loop.
  2) The basic idea for unroll the while loop is to devide the process into three phases, use
     a variable called UnrollCnt to denote the different phases; UnrollNum is the total number
     of unroll need to do.
    a) initialization phase (UnrollCnt = 0): compile the B and S once, and generate the label 
       for the start and end of the loop; put the start label and termination label.
    b) unroll phase ( 1 <= UnrollCnt <= UnrollNum): put in the code B and S (without the labels)
    c) final phase (UnrollCnt = UnrollNum): clean up and reset UnrollCnt
     Note that when UnrollNum = 0 is special. In this case I just need to compile it normally.
  3) However, this approach would lead to a problem that.. for nested while loop, the interal
     loop's label would be repeated several times, becase of repeation of the outer loop.
     Therefore, what I did was to remove the duplicated labels, because the label is in the form
     of: /\nL[0-9]+:/. However, because currently the labels are implemented as a list: 
     ['\n',Label,':'], and is only concatinated when need to print it out. It's possible to
     match the pattern of label-list out of the final list, but a bit tedious.
     What I did here is to create the full label when first generating them, using predicate:
     atomic_list_concat(['\n',Label,':'],FullLbl), and put the FullLbl into the code.
     However, because that atomic_list_concat can only accept bounded values.. I changed all
     parts of the code to be: generate the label first, and then use them. This caused some
     changes in the switch case, because the old implementation is to generate the list of
     unbound variables, and bound them later on in the predicate 'casetable'.
  4) There's something more I did (which is not necessary): when generating the code, initially
     We used atomic_list_concat to link all the code, like: 
        atomic_list_concat([SCode,BCode,Label],Code)
     but because that for one time(not in the solution) I want to keep label in the code 
     unbound so that I can use term_copy to make different copies of the code with different
     labels. But atomic_list_concat do not support the unbound code.. so I changed all these
     places using atomic_list_concat into a list cancatination, like:
        Code = [SCode,BCode,Label]
     This would create nested list, so I created a predicate atomic_list_recur_to_single_list,
     to take out all the nested elements into a single list, after the compiliation is done.
  5) Finally, I use predicate remove_duplicate_lbl to remove the duplicate label with format
     /\nL[0-9]+:/
  6) Oh.. as a plus, the code here also support break inside the switch case.
...
cs((if B then { S1 } else { S2 }), Code,Ain,Aout) :- !,
    ...
    get_assoc(labelsuffix,A2,Kin,A3,Kout),
    generateLabels([Lthen,Lifend],Kin,Kout),

    COpB = [ '\n\t\t ',I,' ',Lthen ],
    cs({S2},C2,A3,A4),      % generate code for 'else', and jump to skip the 'then'
    atomic_list_concat(['\n',Lthen,':'],LblThenStart),
    Cif1 = [ '\n\t\t jmp ',Lifend,LblThenStart ],
                            % label for 'then' branch is 'Lthen:'
    cs({S1},C1,A4,Aout),      % generate code for 'then' branch
...
cs((for (X1=E1;B;X2=E2) do { S }), Code,Ain,Aout) :- !,
    get_assoc(labelsuffix,Ain,Kin,A0,Kout),
    generateLabels([NewLoopStart,NewLoopMain,NewLoopEnd],Kin,Kout),
	                                 % Fill placeholders with concrete label names
                                     
    get_assoc(loop_start_lbl,A0,OldLoopStart,A1,NewLoopStart),
    get_assoc(loop_end_lbl,A1,OldLoopEnd,A2,NewLoopEnd),
	                                 % Handle 'break' and 'continue' attrs similar to 'while' rule
	cs(X1=E1,CInit,A2,A3),              % compile first assignment
    atomic_list_concat(['\n',NewLoopStart,':'],LblLoopStart),
    CLoopNeck = [ '\n\t\tjmp ',NewLoopMain,LblLoopStart], % Generate 'top-of-loop' label
	cs(X2=E2,CUpdate,A3,A4),              % Compile updating assignment
	put_assoc(context,A4,stmt,A5),   % Compile boolean condition in statement context
    atomic_list_concat(['\n',NewLoopMain,':'],CLblMain),
    comp_expr(B,CB,A5,A6),
    ...
    COpB = [ '\n\t\t ',I,' ',NewLoopEnd ],
    cs(S,CS,A6,A7),                  % compile body of for loop
...
cs((N::{S} ; Rest),Code,Ain,Aout) :- !,
    ...
    get_assoc(labelsuffix,Ain,LNin,A0,LNout),
    generateLabels([L],LNin,LNout),
    get_assoc(case_table_labels,A0,(VL,LL),A1,([N|VL],[L|LL])),
...
cs((default::{S}),Code,Ain,Aout) :- !,
    ...
    get_assoc(labelsuffix,Ain,LNin,A0,LNout),
    generateLabels([L],LNin,LNout),
    get_assoc(case_table_labels,A0,(VL,LL),A1,([default|VL],[L|LL])),
...
cs(switch E of { CaseList },Code,Ain,Aout) :-
    ...
    get_assoc(labelsuffix,Ain,Lin,A_1,Lout),
    generateLabels([Lcasetab,Lcaseend],Lin,Lout),

    get_assoc(loop_end_lbl,A_1,OldLoopEnd,A0,Lcaseend),
    get_assoc(label_case_end,A0,OldCaseLabel,A1,Lcaseend),
    ...
    get_assoc(case_table_labels,A1,OldCaseTableLabels,A2,([],[])),
    ...
    put_assoc(context,A2,expr,A3),
    comp_expr(E,CE,A3,A4),			 % compile E in expr context
    get_assoc(expr_result,A4,RE),    % retrieve the result storage for E
    (    RE = [Reg|_]
     ->  Cop = [],
         member((Reg,X),
                [ (0,'%eax'),(1,'%edx'),(2,'%ecx'),
    ...
    put_assoc(case_table_labels,A7,OldCaseTableLabels,A8),
...
casetable(Code,Ain,Aout) :-
    get_assoc(case_table_labels,Ain,(CTV,CTL),Aout,none),
							% get the association lists, and replace them with 'none' at the same time
                            % Labels are bounded already
