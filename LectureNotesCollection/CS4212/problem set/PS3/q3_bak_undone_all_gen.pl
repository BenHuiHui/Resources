...
generateLabels([H|T],LabelSuffixIn,LabelSuffixOut) :-
    atomic_list_concat_recur(['L',LabelSuffixIn],H),
...
local_vars_helper(V,Ain,Aout) :-
	...
	TopOut #= TopIn + 4, atomic_list_concat_recur([-,TopOut,'(%ebp)'],Ref),
...
cs((while B do { S }), Code,Ain,Aout) :- !,
    ...
    get_assoc(loop_start_lbl,Ain,OldLoopStart,Ax,ThisLoopStart),
    get_assoc(loop_end_lbl,Ain,OldLoopEnd,Ax,ThisLoopEnd),

    get_assoc(while_nest_lvl,Ax,WhileNestLvl,Ax,NewWhileNestLvl),NewWhileNestLvl is WhileNestLvl + 1,
    get_assoc(nested_loop_start_end_lbl_pair,Ax,LoopStartEndAssoc),
    (   get_assoc(WhileNestLvl,LoopStartEndAssoc,(ThisLoopStart,ThisLoopEnd))
    ->  Ax = Ax                                     % encounter before
    ;   get_assoc(labelsuffix,Ax,LxIn,Ax,LxOut),    % first encounter 
        generateLabels([ThisLoopStart,ThisLoopEnd],LxIn,LxOut),
        put_assoc(WhileNestLvl,LoopStartEndAssoc,(ThisLoopStart,ThisLoopEnd),NewLoopStartEndAssoc),
        put_assoc(nested_loop_start_end_lbl_pair,Ax,NewLoopStartEndAssoc)
    ),

    get_assoc(unroll_cnt_assoc,Ax,UnrollCntAssoc,NewUnrollCntAssoc,Ax),
    (   get_assoc(WhileNestLvl,UnrollCntAssoc,UnrollCnt) ->  true ;   UnrollCnt = 0)
    NewUnrollCnt is UnrollCnt + 1, put_assoc(WhileNestLvl,UnrollCntAssoc,NewUnrollCnt,NewUnrollCntAssoc),
    ...
    (   (   UnrollNum = 0,!,
        ...
        (   UnrollCnt = 0,!,
            cs( while (B) { S }, RestCode, Ax, Ax),
            Code = [
                '\n',ThisLoopStart,':',
                BCode,SCode,RestCode],
        (   UnrollCnt < UnrollNum,!,
    ...
    put_assoc(loop_start_lbl,Ax,OldLoopStart,Ax),
    put_assoc(loop_end_lbl,Ax,OldLoopEnd,Aout).
...
compile(P,File,UnrollNum) :-
	...
	empty_assoc(UnrollCntAssoc),
	AUnrollCntAssocIn = AUnrollNumOut,
    put_assoc(unroll_cnt_assoc,AUnrollCntAssocIn,UnrollCntAssoc,AUnrollCntAssocOut),

    AWhileNestLvlIn = AUnrollCntAssocOut,
    put_assoc(while_nest_lvl,AWhileNestLvlIn,0,AWhileNestLvlOut),
    AstartIn = AWhileNestLvlOut,
	...
	atomic_list_concat_recur(All,AllWritable),     % Now concat and get writable atom
	...
	atomic_list_concat_recur(VarCode,WritableVars),
	...
	atomic_list_concat_recur(VarNames,WritableVarList),
	...
	atomic_list_concat_recur(VarPtrs,WritableVarPtrs),
