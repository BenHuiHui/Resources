...
scan_label([],A,A):-!.
scan_label([H|T],Ain,Aout):-!,
  scan_label(H,Ain,Aaux), scan_label(T,Aaux,Aout).
scan_label(P,Ain,Aout):-!,
  P =.. [Op|T],
  ( Op = (::),T = [indexlabel(N)|S]
  ->  
      get_assoc(labelsuffix,Ain,LabelSuffixIn,A1,LabelSuffixOut),
      generateLabels([LabelInternal],LabelSuffixIn,LabelSuffixOut),
      get_assoc(labelmapping,A1,LabelMapping,A2,[(N,LabelInternal)|LabelMapping]),
      scan_label(S,A2,Aout)
  ;   scan_label(T,Ain,Aout)
  ).
removelast3([],[]):-!.
removelast3([H|T1],[H|T2]):-removelast3(T1,T2).

cs_goto_unwrap_code([],[]):- !.
cs_goto_unwrap_code([(N,Lbl)|T],[CH|CT]):-
    CH = ['\n\t\t cmpl $',N,',%eax',
          '\n\t\t je ',Lbl],
    cs_goto_unwrap_code(T,CT).
...
cs(Expr,Code,Ain,Aout) :- 
    ( Expr = (indexlabel(N) ::), CodeS = [''], Aout = Ain 
    ; Expr = (indexlabel(N) :: S), cs(S,CodeS,Ain,Aout)),!,
    get_assoc(labelmapping,Aout,LabelMapping),
    member((N,LabelInternal),LabelMapping),
    append([ ['\n',LabelInternal,':'],CodeS], Code).

cs((goto Expr),Code,Ain,Aout) :- !,
    get_assoc(labelmapping,Ain,LabelMapping),
    put_assoc(context,Ain,stmt,A1),
    comp_expr(Expr,CodeETmp,A1,Aout),    % the result would be stored in %eax
    removelast3(CodeETmp,CodeE),        % the last expression is cmp, no use here.
    cs_goto_unwrap_code(LabelMapping,CodeUnwrapList),
    append(CodeUnwrapList,CodeUnwrap),
    append([CodeE,CodeUnwrap],Code).
...
compile(P,File) :-
	...
	ALoopStartIn = Empty,
    put_assoc(loop_start_lbl,ALoopStartIn,none,ALoopStartOut),
	AendIn = ALoopStartOut,                     % initial 'break' label is none
    ...
    ALblMappingIn = AmaxlocalOut,
    % NOTE - scan the label once
    put_assoc(labelmapping,ALblMappingIn,[],ALblMappingOut),          % add initial label counter
    AScanLblIn = ALblMappingOut, scan_label(P,AScanLblIn,AScanLblOut),
    % NOTE - end of editing
